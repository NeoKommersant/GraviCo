  ...
  useEffect(() => {
    ...
    function applyDrag() {
      ...
      const angleY = isHorizontal
        ? (rX / maxDrag) *
        maxTiltY *
        (1 + (Math.abs(rX) / maxDrag) * foldFactor) *
        0.15
        : 0;
      const brightness =
        1 -
    ...
    function onTouchEnd() {
      ...
      if (horizSwipe && subSection > 0 && newSec !== section)
    ...
    // Анимация «прыжка»/телепорта при смене секции
    ...
    if (teleportRef.current) {
      ...
      // Если свайп горизонтальный, можно заменить ease на 'elastic.out(...)'
      const easeType =
        isHorizontal && prev.section !== section
          ? 'elastic.out(0.1, 1)'
          : 'power3.out';
        ...
        <section className="flex-shrink-0 w-screen flex flex-col">
          <div className="h-screen p-1">
            <div className="w-full h-full bg-[#2C2F3A] rounded-xl shadow-lg flex items-center justify-center">
              <h1
                className="text-3xl md:text-4xl font-bold text-[#F4F5F7]"
          ...
          <div className="h-screen p-1">
            <div className="w-full h-full bg-[#4A4E61] rounded-xl shadow-lg flex items-center justify-center">
              <h2
                className="text-2xl md:text-3xl font-semibold text-[#E8DED2]"
                style={{ fontFamily: 'Manrope' }}
              >
                Премиум решения
              </h2>
          ...
          <div className="h-screen p-1">
            <div className="w-full h-full bg-[#4A4E61] rounded-xl shadow-lg flex items-center justify-center">
              <h2
                className="text-2xl md:text-3xl font-semibold text-[#D4AF37]"
                style={{ fontFamily: 'Manrope' }}
              >
